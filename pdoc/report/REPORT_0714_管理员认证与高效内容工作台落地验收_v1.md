...
> 2026-07-15 远端收口：已完成远端 migration、运行时冲突定位、Free Plan v2 / 100000 零流量验收、真实 Secret 轮换、100% 发布、真实浏览器登录、退出与重放隔离。运行时冲突证据见 `pdoc/report/REPORT_CONFLICT_0714_Cloudflare管理员密码派生运行时限制_v1.md`。
...
- 范围外：Cloudflare Access 策略；当前生产使用 `application_session`
...
远端完整 v2 候选和真实 Secret 版本都已通过 0% Version Override 门禁，合法形状假凭证返回 401，未出现 1102；生产已切换到真实版本 `42de0530-dbe4-4843-8c95-ed9f8576bcd8` 单版本 100%。上线后健康、Origin、请求校验、无会话和受保护文档 smoke 全部符合预期。Solazhu 已完成真实浏览器登录，确认工作台与管理员身份；退出后浏览器后退和刷新均不能恢复私有工作台。D1 最终为 1 条已撤销会话、0 条活动会话、0 条限流记录，会话只保存 64 位小写十六进制 token hash。

## 3. 实施结果

### 3.1 配置型管理员与终端初始化

- `ADMIN_AUTH_CONFIG` 新配置使用固定 `v2.<base64url JSON>` 格式；遗留 v1 仍严格表示 600000 次。配置包含账号、显示名、随机 `credentialId`、PBKDF2 参数、salt、verifier 和随机限流密钥，不包含原始密码。
- 新配置 PBKDF2-HMAC-SHA256 固定 100000 次；错误账号和错误密码统一执行完整校验并返回同类错误。
...
- 远端初始化仅通过标准输入调用 `wrangler versions secret put ADMIN_AUTH_CONFIG`，创建未部署候选版本；配置值不会进入 argv。
...
- 完整桌面布局的管理员身份与退出动作位于左侧栏底部；窄侧栏和移动布局才迁移到顶部栏，两处不同时显示。
...
| 全量单元测试 | 通过 | 61 files / 535 tests |
| 全量组件测试 | 通过 | 19 files / 244 tests |
| 管理员初始化定向测试 | 通过 | 1 file / 15 tests；包含 v2 互操作、原子写入、权限、非 TTY、stdin Secret、LF/CRLF 与无 EOF 换行 |
...
| 本地 PBKDF2 基准 | 通过 | v2 / 100000 共 100 组成功与失败；成功 P50/P95 10.53/14.06ms，失败 P50/P95 10.63/14.10ms，均低于本地 1 秒门禁 |
...
1. D1 migration 0010 已应用，当前无待应用 migration。
2. v2 / PBKDF2-HMAC-SHA256 / 100000 的固定探针、完整合成候选和真实 Secret 版本均在真实边缘完成；未出现 1101、1102 或 5xx。
3. 真实 Secret 版本先以稳定版 100% + 候选 0% 验证，再切为单版本 100%；生产版本为 `42de0530-dbe4-4843-8c95-ed9f8576bcd8`。
4. 生产 `APP_ORIGIN`、`ADMIN_BROWSER_AUTH_MODE=application_session`、D1、Assets 和两个 Secret 绑定完整。
5. Solazhu 真实登录后，D1 恰好出现 1 条活动会话；token hash 为 64 位小写十六进制，会话有效期 480 分钟，成功登录后限流为 0。
6. Solazhu 从左侧栏底部退出后，D1 会话变为 revoked、活动会话为 0；浏览器后退和刷新不能恢复工作台。原始 Cookie 未被记录或输出；相同构建的隔离整栈测试另行覆盖原始旧 Cookie 重放并返回 401。
7. Workers Free 未配置 `limits.cpu_ms`，未引入 Durable Object、Wasm、Access 或外部认证服务。
...
- 远端 Secret、migration、变量、零流量候选与 100% 部署：通过。
- 生产发布：GO；管理员登录、工作台、退出、撤销和后退/刷新隔离闭环完成。
